...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                  ...
                  if( maxval( abs(newgap-gap) ) > acc .or. iter==maxiter) then
                     ...
                     write(*,*) iter,maxval(abs(newgap-gap))
                     writegap=.false.
                     dgap=0_long
                  endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          SUBROUTINE READXI
            ...
            if(rho .ge. 0.02d0 .and. mod(rho*1000d0,10d0) .ne. 0d0) then
               write(rhoc,'(SS,F5.3)') rho
            endif

            if( temp > 0.d0 ) then
              write(tchar,'(SS,f3.1)') temp
              if( mod(temp*100d0,10d0) .gt. 0d0) then
                write(tchar,'(SS,f4.2)') temp
              endif
              write(*,*) temp,tchar,mod(temp*100d0,10d0),temp*100d0
